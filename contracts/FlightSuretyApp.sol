...
contract FlightSuretyApp {
    ...
    */
    ...
    event OracleRequest(address airline, string flight, uint256 timestamp, bool isOpen);
    ...
    */   
    function registerAirline(address account) external {
        if (flightSuretyData.totalAirlines() < 4) {
            require(flightSuretyData.firstAirline() == msg.sender, "Until 4 airlines are registered only first airline registered can register new ones");
            flightSuretyData.registerAirline(account);
        } else {
            bool isDuplicate = false;
            for(uint i = 0; i < votes[account].length; i++) {
    ...
    *
    */  
    function registerFlight(string flight, uint price, uint256 timestamp) external {
    ...
    */  
    ...
    function fetchFlightStatus(address airline, string flight, uint256 timestamp) external {
        ...
        emit OracleRequest(airline, flight, timestamp, oracleResponses[key].isOpen);
    }

    function buy(address airline, string flight, uint256 timestamp, uint amount) external valWithinRange(amount, 0, 1.05 ether) requireIsOperational payable {
...
// region ORACLE MANAGEMENT
    ...
    function getFlightKey(address airline, string flight, uint256 timestamp) public pure returns(bytes32) {
...
contract FlightSuretyData {
    ...
    function totalAirlines() external view returns (uint);
    function firstAirline() external view returns (address);
    function registerFlight(string flight, uint price, uint timestamp, address airline) external;
